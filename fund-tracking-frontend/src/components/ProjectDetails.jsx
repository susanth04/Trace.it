...
function ProjectDetails() {
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      try {
        ...
        setIsOwner(
          account.toLowerCase() === projectData.projectOwner.toLowerCase()
        );
  ...
  const handleSpendFunds = async (e) => {
    ...
    try {
      ...
      // Spend funds using the contract
      await contract.methods
        .spendFunds(id, amountInWei, spendDescription, receiver)
        .send({ from: account });

      // Refresh project data
      const updatedProjects = await contract.methods.getProjects().call();
